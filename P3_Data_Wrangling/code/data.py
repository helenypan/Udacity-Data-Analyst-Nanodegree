...
expected = ["Street", "Avenue", "Boulevard", "Drive", "Court", "Place", "Square", "Lane", "Road", 
            ...
            "Gardens","Walk","Park","Mount","View","Parade","Headgate","Rayleighs","Mews","Bury","Marina",
            "Green","Meadway","Vale","Rise","Middleborough","Heath","World","West","East"
            ]
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for tag in element.iter("tag"):
            cur_tag = get_tag_dict(node_attribs["id"],tag)
        ...
        for tag in element.iter("tag"):
            cur_tag = get_tag_dict(way_attribs["id"],tag)
            if cur_tag is not None:
                tags.append(cur_tag)
        node_counter = 0
        for node in element.iter("nd"):
            cur_node = {}
            cur_node["id"] = way_attribs["id"]
            cur_node['position'] = node_counter
            cur_node['node_id'] = node.attrib["ref"]
            way_nodes.append(cur_node)
            node_counter += 1
